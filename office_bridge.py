...
- Стоп за найдальшою зоною маніпуляції біля входу
  ...
  Відкат у сильну свічку (ICT SMC HUNTER v9.9): volume>SMA20×2 і range>ATR14×1.2,
  зона high–low, OTE 0.618–0.786, вхід 60% на 0.618 і добір 40% на 0.786.
  Стоп: lowest(low,7)−2×ATR / highest(high,7)+2×ATR; люфт Герчика лише якщо ширше.
  Хвилини маніпуляції 09:00–09:15 і 15:00–15:15 Київ — сигнал не надсилати.
  Азія 03:00–07:00 Київ. TP: MO → Asian → PDH/PDL → W High/Low (запас 1.5R/3R/5R).
...
- ATR >80% = ПРОПУСК конкретного входу по тренду (правило Герчика).
  ...
  NO_TRADE 100% у Probability Engine = спрацювало правило ATR>80, не ймовірність що ціна не рухається.
  Нова денна свічка лише перераховує day_used = діапазон/ATR; ATR(14) не reset і не авто-вхід.
...
НЕ ТОРГУВАТИ якщо:
- ATR day_used > 80% по тренду без sweep/ЛП — конкретний вхід, не заборона шукати нову структуру
...
ЯКЩО ПРОПУСК:
Причина: назви правило (ATR/Edge/свіжість/структура), не «бак порожній» і не «ймовірність 100% збитку».
NO_TRADE 100% = спрацював hard gate, не прогноз угоди.
Зараз: УГОДИ НЕМАЄ — якщо немає підтвердженої альтернативи на свічках.
Заборонено рядок «Чекаю зону» з цифрами, яких немає у свічках.
Не підставляй азійський рендж бота і не підставляй добір бота як зону офісу.
Нова денна свічка не є входом і не обнуляє ATR(14).
